...
#Wykonal: Jan Fedorowicz(dynks1)©Copyrights by Programy.Dynks1.pl (2018-2021)all rights reserved
...
class MojFormularz(klasa_bazowa, klasa_formularza):
    def __init__(self, parent=None):
        ...
        # Laczenie przyciskow z zdarzeniami ktore wykonuje
        ...
        plikUchwyt = True
        try:    # Otwierane plikow
            plik = open("zapisane_dane_wz.pkl", "r")
        except:
            plikUchwyt = False
        if plikUchwyt:
            zapis_lista = pickle.load(plik) # Odczyt pliku
            plik.close()
        else:       # KIedy nie ma pliku, to ustawiane sa wartosci domyslne
        ...
        # Ile czasu minelo od uruchomienia programu
        roznica = datetime.datetime.now() - znacznik_czasu_wtedy
        tykniecia = roznica.seconds / 50

        # Symululacja wszystkich tykniêc, które nast¹pi³y od momentu ostatniego uruchomienia programu
        ...
        # Dobieranie odpowiedniej animacji      
        if self.sen:
...
u"Czy na pewno chcesz obudziæ zwierzaka? Bêdzie bardzo niezadowolony!",
    ...
    def animacja_timer(self):
        ...
        # Aktualizacji rysunku zwierzaka
    ...
    def tykniecie_timer(self):
        ...
        # W zale¿noœci od aktywnoœci dodawania lub odejmowanie wartosci    
        if self.doktor:
            self.zdrowie += 1
            self.glod += 1
        ...
        # Sprawdzanie, czy wartoœæ znajduje siê w okreœlonym przedziale        
